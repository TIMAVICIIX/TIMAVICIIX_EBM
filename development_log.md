...
## <br>2025.05.21
#### 暂不考虑优化自动填充，此方式会提高代码耦合性，组合声明即可，有类型报错的
#### 正在剥离书本UI界面与原版bookData的代码逻辑，优化UI逻辑内聚，最后将结构化书本存储模板完全替代bookData
#### 书本UI界面应保持高度独立性，完善并拆解组合自身业务逻辑，隔离数据逻辑，通过接口交流即可
#### 书本UI应存在两种状态，并依赖书本状态改变自身状态，这将简化开发进程

## <br>2025.05.22
#### 翻看前几个月的开发状况，实在令人难绷，基于原版组件开发的UI界面与UI组件之间到处耦合，硬编码字符乱飞
#### 1.现在首要将组件与UI界面解耦，将多余的内联函数剔除，优化多余的回调机制
#### 2.其次将UI界面内部解耦，把硬编码配置归类合并，创建高效简洁的UI声明方式
#### 3.最后将老版数据结构bookData与UI解耦并剥离，达成昨日日志中目标

## <br>2025.05.23
#### 评估后书本UI优化方案：
#### 1.抽取内容编辑界面缓存逻辑与部分触发逻辑，建立缓存对象到UI界面内组合
#### 2.该缓存对象负责收集左右界面内容，自动处理左右界面文本注入，保存界面内容缓存
#### 3.在此基础上应存在剪贴板粘贴逻辑处理，识别粘贴目标，计算渲染行数，将剩余行数再次进行计算
#### 4.将多余行数切割，计算预演页数，最后保存所有内容，呈现末尾内容到UI界面
#### 5.总之，内容缓存对象的主要两个功能是：防止界面拉伸重渲染时内容丢失与处理文本粘贴内容

#### 原版粘贴逻辑:
#### 按键监听 → 特殊按键监听 → 粘贴按键监听 → 
#### 获取剪贴板文本并加入组件text → 移动光标索引 → 唤起更改监听函数 → 其他监听操作 → 
#### 重新整理行数,放入内容渲染缓存 → 通过rendererButton函数按游戏刻在UI界面进行渲染

#### 目标更改后逻辑：
#### 按键监听 → 特殊按键监听 → 粘贴按键监听 → 
#### 获取剪贴板文本 → 唤起UI内容缓存器 → 进行粘贴内容处理（判断左右页，整理行数，计算末尾页） →
#### 分页缓存粘贴内容 → 末尾页分发到左右页UI → 移动光标索引 → 唤起更改监听函数 → 其他监听操作 →
#### 再次整理行数,放入内容渲染缓存 → 通过rendererButton函数按游戏刻在UI界面进行渲染

## <br>2025.05.24
#### 优化评估方案：取消标记式分片文本存储，拟采用整体存储 + 富文本标记(MarkDown)进行文档保存
#### 考虑到渲染通过行数进行，并且页数不固定，因此采用整体存储法，将富文本标记与文本合并存储
#### 文本存储由以下三个存储结构构成：1.文本（富文本标记+文本颜色索引（颜色起始位+终止位）+目录索引）

## <br>2025.05.25
#### 评估方案序：要达成上述评估方案，底层数据结构可能需要切换到JSON格式，也就是说底层数据逻辑也需要重构
#### XML作为备选导出格式，JSON作为底层持久化格式，保留文本的同时保留颜色，格式与索引信息。
#### 使用原版NBT存储机制，设计分支NBT，大文本通过NBT导出

## <br>2025.05.28
#### Nbt存储机制改造完成，屏蔽掉异步模块后进行了测试。
#### 测试评估：
#### 1.Nbt存取机制稳定无异常
#### 2.Block的委托注册改造后存在侵入式注册问题
#### 3.之前的BlockItem注册返回也存在数据返回被擦除情况
#### 4.拟继续改造注册流程，解耦各Block的注册逻辑

## <br>2025.05.29
#### 侵入式注册Bug修正完成，侵入式注册是由于委托的懒加载导致
#### 后期采用KProperty还原注册逻辑时失败，出现推断递归，暂使用字符串ID注册
#### 将恢复书本的Tooltip功能
#### 将进行网络部分UI开发
#### 将测试异步网络模块
#### 将进行富文本解析器开发
...
## <br>2025.06.01
#### 已修复书本Tooltip功能，通过Mixin与tooltip两个监听器触发item的重命名
#### 顶部提示框开发完成，书本加载遮罩开发完成
#### 即将完善网络模块，并进行异步测试
#### 后续进行富文本解析器开发

## <br>2025.06.02
#### 异步网络模块基础测试非常成功!!!几乎一次到位
#### 接下来将持续完善模块逻辑，并为内容解析器进行架构设计
#### 异步网络模块目前还缺少服务端的验证测试，明天做完就可以进行下一步了

## <br>2025.06.03
#### 进行异步网络模块的边缘测试，进行存储后的数据完整性测试，最后完成UI健壮性补全
#### 目前采取 子数据结构属性注册到父数据结构中统一管理的方法进行数据的批量处理，但可能会出现读写不一致问题，正在尝试解决
#### 读写不一致问题无中生有，目前对数据属性前置了懒加载的强制加载总线，在数据模板相对Nbt和外部存储IO时会激活强制实例化总线，为未使用的属性进行强制加载
#### 边缘测试与健壮性测试完成，UI健壮性补全完成，存储完整性测试完成
#### 即将进行富文本解释器架构设计 

## <br>2025.06.05
#### 近两天在设计富文本架构解释器时，评估了Markdown的可用性，，稳定性，易实现性与用户友好性
#### 1.Markdown在便携写作中确实占据优势，但在普及方面较弱
#### 2.另外Markdown语法不稳定，不能很好衔接其他格式内容，只能做简单注释，在自定义语法方面缺乏扩展性
#### 3.Markdown解析流程复杂，涉及嵌套解析，字符转义解析和解析优先级
#### 4.因此评估Markdown语法不适用于解析器核心逻辑，拟采用XML语法，通过界面UI与热键对数据内容进行格式解释
#### 此外需要研究自主EditText组件的可行性，摆脱原版傀儡组件，为文本呈现提供更高的灵活性

## <br>2025.06.06
#### 三层XML解释+渲染架构涉及方案：
#### 1.设计三层模块架构分别为:内容解释层，桥接分发层，UI渲染层
#### 2.桥接分发层为模块核心，负责上下层次桥接，同时提供事件内容缓存，数据内容分发功能
#### 3.内容解释层提供解释接口，UI渲染层提供渲染解释接口
#### 4.桥接分发层向上实现UI渲染逻辑，向下实现内容解释逻辑
#### 5.内容解释层提供解释管理，UI渲染层提供渲染管理，两个层次均须暴露管理设定接口
#### 6.最终形成以桥接分发层为中心，数据流从下往上，事件流从上往下的中心化架构

## <br>2025.06.11
#### 内容解释层采用了父解释器框架+子解释器实现逻辑的思路
#### 提供静态对象的解释器队列或单个解释器
#### 解释器内部存在自定义输出数据结构
#### 桥接分发层将对接渲染逻辑，并预演渲染排版，最后分为多个内容板块
#### 后期也可考虑内容的懒加载策略