...
class RefinedLeatherBookBlock : BaseDirectBlock(GlobalData.getGeneralBlockSetting(DyeColor.BROWN)),
    BlockEntityProvider, BookBlockInterface {

    override fun createBlockEntity(pos: BlockPos?, state: BlockState?): BlockEntity? {
        return if (pos != null && state != null) {
            GeneralBookBlockEntity(BookRegister.EntityTypes.GENERAL_REFINED_LEATHER_BOOK_TYPE, pos, state)
        } else {
            null
        }
    }
