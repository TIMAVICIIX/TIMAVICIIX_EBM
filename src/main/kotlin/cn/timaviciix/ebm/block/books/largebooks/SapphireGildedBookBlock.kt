...
class SapphireGildedBookBlock : BaseBookBlock(BookType.LargeBook),
    BlockEntityProvider, BookBlockInterface {
    override fun createBlockEntity(pos: BlockPos?, state: BlockState?): BlockEntity? {
        return if (pos != null && state != null) {
            LargeBookBlockEntity(LargeBookRegister.EntityTypes.LARGE_SAPPHIRE_GILDED_BOOK_TYPE, pos, state)
        } else {
            null
        }
    }
