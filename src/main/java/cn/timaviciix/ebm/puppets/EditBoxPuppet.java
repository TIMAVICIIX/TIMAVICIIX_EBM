...
public class EditBoxPuppet {
    ...
    private final List<EditBoxPuppet.Substring> lines = Lists.newArrayList();
    ...
    public EditBoxPuppet(TextRenderer textRenderer, int width) {
    ...
    public void replaceSelection(String string) {
        if (!string.isEmpty() || this.hasSelection()) {
            ...
            EditBoxPuppet.Substring substring = this.getSelection();
     ...
     */
    public EditBoxPuppet.Substring getSelection() {
        return new EditBoxPuppet.Substring(Math.min(this.selectionEnd, this.cursor), Math.max(this.selectionEnd, this.cursor));
    }
    ...
    public int getCurrentLineIndex() {
        for (int i = 0; i < this.lines.size(); i++) {
            EditBoxPuppet.Substring substring = this.lines.get(i);
     ...
     */
    public EditBoxPuppet.Substring getLine(int index) {
    ...
    public void moveCursorLine(int offset) {
        if (offset != 0) {
            ...
            EditBoxPuppet.Substring substring = this.getOffsetLine(offset);
    ...
    public void moveCursor(double x, double y) {
        ...
        EditBoxPuppet.Substring substring = this.lines.get(MathHelper.clamp(j, 0, this.lines.size() - 1));
     ...
     */
    public Iterable<EditBoxPuppet.Substring> getLines() {
        return this.lines;
    }
     ...
     */
    private EditBoxPuppet.Substring getCurrentLine() {
     ...
     */
    private EditBoxPuppet.Substring getOffsetLine(int offsetFromCurrent) {
     ...
     */
    @VisibleForTesting
    public EditBoxPuppet.Substring getPreviousWordAtCursor() {
        if (this.text.isEmpty()) {
            return EditBoxPuppet.Substring.EMPTY;
        } else {
            ...
            return new EditBoxPuppet.Substring(i, this.getWordEndIndex(i));
        }
    }
     ...
     */
    @VisibleForTesting
    public EditBoxPuppet.Substring getNextWordAtCursor() {
        if (this.text.isEmpty()) {
            return EditBoxPuppet.Substring.EMPTY;
        } else {
            int i = MathHelper.clamp(this.cursor, 0, this.text.length() - 1);
     ...
     */
    public void reWrapInLines() {
        this.lines.clear();
        if (this.text.isEmpty()) {
            this.lines.add(EditBoxPuppet.Substring.EMPTY);
        } else {
            this.textRenderer
                    .getTextHandler()
                    .wrapLines(this.text, this.width, Style.EMPTY, false, (style, start, end) -> this.lines.add(new EditBoxPuppet.Substring(start, end)));
            if (this.text.charAt(this.text.length() - 1) == '\n') {
                this.lines.add(new EditBoxPuppet.Substring(this.text.length(), this.text.length()));
            }
        }
    ...
    /**
     * @param massivelyText 包含本页内容+剪贴板内容
     * @return 返回计算后行数与每行起始结束位置，后续交由内容缓存器处理
     */
    public List<EditBoxPuppet.Substring> reWrapBeforeLines(@NotNull String massivelyText) {
        List<EditBoxPuppet.Substring> resultList = new ArrayList<>();
        if (massivelyText.isEmpty()) {
            return resultList;
        } else {
            this.textRenderer
                    .getTextHandler()
                    .wrapLines(
                            ...
                            Style.EMPTY, false,
                            ((style, start, end) -> resultList.add(new EditBoxPuppet.Substring(start, end))));
            // 末尾若存在换行符，自动加一行
            if (this.text.charAt(this.text.length() - 1) == '\n') {
                resultList.add(new EditBoxPuppet.Substring(massivelyText.length(), massivelyText.length()));
            }
        }
        return resultList;
    }
    ...
    public record Substring(int beginIndex, int endIndex) {
         ...
         * An empty substring.
         */
        static final EditBoxPuppet.Substring EMPTY = new EditBoxPuppet.Substring(0, 0);
    }
}
