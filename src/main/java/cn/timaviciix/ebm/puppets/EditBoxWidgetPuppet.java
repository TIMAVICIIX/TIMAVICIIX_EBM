...
public class EditBoxWidgetPuppet extends ScrollableWidget {
    ...
    public final EditBoxPuppet editBox;
    private int tick;

    public EditBoxWidgetPuppet(TextRenderer textRenderer, int x, int y, int width, int height, Text placeholder, Text message) {
        super(x, y, width, height, message);
        this.textRenderer = textRenderer;
        this.placeholder = placeholder;
        this.editBox = new EditBoxPuppet(textRenderer, width - this.getPaddingDoubled());
    ...
    protected void renderContents(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (string.isEmpty() && !this.isFocused()) {
            ...
            for (EditBoxPuppet.Substring substring : this.editBox.getLines()) {
            ...
            if (this.editBox.hasSelection()) {
                EditBoxPuppet.Substring substring2 = this.editBox.getSelection();
                int m = this.getX() + this.getPadding();
                l = this.getY() + this.getPadding();

                for (EditBoxPuppet.Substring substring3 : this.editBox.getLines()) {
    ...
    private void onCursorChange() {
        ...
        EditBoxPuppet.Substring substring = this.editBox.getLine((int)(d / 9.0));
        if (this.editBox.getCursor() <= substring.beginIndex()) {
            d = this.editBox.getCurrentLineIndex() * 9;
        } else {
            EditBoxPuppet.Substring substring2 = this.editBox.getLine((int)((d + (double)this.height) / 9.0) - 1);
